    ...
    return (
        <div className='flex flex-col items-start justify-start h-screen w-screen'>
            <div className="flex flex-row h-2/3 w-full">
                ...
                <div className='flex flex-col justify-start items-center h-full w-1/4 p-2 rounded-xl bg-blue-200'>
                    <div className="flex flex-col w-full h-1/4 overflow-y-scroll">
                        <div className='flex flex-row w-full h-1/3 justify-start items-start '>
                            <KeywordBar options={options}  keywords={keywords}  setKeywords={setKeywords} setOptions={setOptions} />
                        <button className='flex flex-col w-1/2 text-center rounded- bg-green-400 p-2' onClick={()=>{
                            setMonthState(monthState + 1 )
                        }}> Next Month</button>
                        </div>
                    </div>
                    <div className=' h-3/4 w-full p-2 text-white overflow-y-scroll'>
                        <KeywordListDisplay  keywordData ={keywordData}/>
                    </div>
                </div>
            </div>
            <div className="flex h-1/3 w-full bg-gray-500 p-8">
                <KeywordList keywordData={keywordData}/>
            </div>
        </div>
    )
}
